...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (isManager(authResult.user) && taskData.assignedTo && taskData.assignedTo.length > 0) {
      const invalidIds = await validateTaskAssignment(authResult.user._id, taskData.assignedTo);
