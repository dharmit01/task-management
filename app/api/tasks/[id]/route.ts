...
/** Resolve a task by either TSK-N human ID or MongoDB ObjectId */
function findTaskById(id: string) {
  if (/^TSK-\d+$/.test(id)) {
    return Task.findOne({ taskId: id });
  }
  return Task.findById(id);
}

/** Resolve a task (with full population) by either TSK-N human ID or MongoDB ObjectId */
function findTaskByIdPopulated(id: string) {
  const query = /^TSK-\d+$/.test(id)
...
const updateTaskSchema = z.object({
  ...
  taskList: z
    .string()
    .regex(/^[0-9a-fA-F]{24}$/, "Invalid task list ID")
    .optional(),
...
export async function GET(
  ...
  try {
    ...
    const task = await findTaskByIdPopulated(id);
    ...
    if (!isAdmin(authResult.user)) {
      ...
      const isAssignee = assignedToArray.some(
        (assignee: any) =>
          assignee._id.toString() === authResult.user._id.toString(),
      );
      const isCreator =
        task.createdBy._id.toString() === authResult.user._id.toString();
...
export async function PATCH(
  ...
  try {
    ...
    const userIsAssignee = assignedToArray.some(
      (assigneeId: any) =>
        assigneeId.toString() === authResult.user._id.toString(),
    );
    const userIsCreator =
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Access denied. You can only edit tasks assigned to you, created by you, or assigned to your team.",
        },
        { status: 403 },
      );
    }
    ...
    if (
      userIsManager &&
      validationResult.data.assignedTo &&
      validationResult.data.assignedTo.length > 0
    ) {
    ...
    const updatedTask = await (
      /^TSK-\d+$/.test(id)
        ? Task.findOneAndUpdate({ taskId: id }, updateData, {
            new: true,
            runValidators: true,
          })
...
export async function DELETE(
  ...
  try {
    ...
    const deletedTask = await (/^TSK-\d+$/.test(id)
      ? Task.findOneAndDelete({ taskId: id })
