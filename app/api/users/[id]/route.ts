import { isAdmin, isManager, requireAdmin, requireAuth, requireManagerOrAdmin } from '@/lib/auth';
...
const updateUserSchema = z.object({
  ...
  annualLeaveBalance: z.number().min(0, 'Leave balance cannot be negative').optional(),
});
  ...
  try {
    ...
    if (isManagerUser && !isAdminUser) {
      ...
      if (!leaveBalanceValidation.success) {
        return NextResponse.json(
          { error: 'Managers can only update leave balance', details: leaveBalanceValidation.error.issues },
          { status: 400 }
        );
      }
