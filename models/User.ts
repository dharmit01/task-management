...
UserSchema.pre('save', function () {
  if ((this.role === 'Manager' || this.role === 'Admin') && this.managerId) {
    const error = new Error('Managers and Admins cannot be assigned to another Manager');
    throw error;
  }
});
...
UserSchema.pre('save', async function () {
  if (this.isModified('managerId') && !this.isNew && this.role === 'Member') {
    try {
      ...
      console.error('Error deallocating tasks during member reassignment:', error);
    }
  }
});
